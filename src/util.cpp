...
int nAnonymizeRampantAmount = 1000;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Rampant
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Rampant
    // Mac: ~/Library/Application Support/Rampant
    // Unix: ~/.Rampant
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Rampant";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "Rampant.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "Rampantd.pid"));
