...
const string strMessageMagic = "Rampant Signed Message:\n";
...
int64_t GetProofOfStakeReward(const CBlockIndex* pindexPrev, int64_t nCoinAge, int64_t nFees)
{
	...
	if (nHeight < 250001) {
		...
		nSubsidy = STATIC_POS_REWARD * 1.25;
	}
	else {
		nSubsidy = STATIC_POS_REWARD * 0.5;	
	}
	return nSubsidy + nFees;
}
...
{
	int64_t ret;
	if (nHeight > 250000 && nHeight < 500001) {
		ret = blockValue * 4 / 5;	//80%
	}
	else {
		ret = blockValue * 3 / 4;	//75%
	}
