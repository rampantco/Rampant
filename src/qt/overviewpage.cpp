...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeRampantAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeRampantAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeRampantAmount = strAnonymizeRampantAmount.remove(strAnonymizeRampantAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeRampantAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeRampantAmount*COIN) nMaxToAnonymize = nAnonymizeRampantAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeRampantAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeRampantAmount));
        strAnonymizeRampantAmount = strAnonymizeRampantAmount.remove(strAnonymizeRampantAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeRampantAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
