...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeRampantAmount"))
        settings.setValue("nAnonymizeRampantAmount", 1000);
    nAnonymizeRampantAmount = settings.value("nAnonymizeRampantAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeRampantAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeRampantAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeRampantAmount:
            return QVariant(nAnonymizeRampantAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeRampantAmount:
            nAnonymizeRampantAmount = value.toInt();
            settings.setValue("nAnonymizeRampantAmount", nAnonymizeRampantAmount);
            emit AnonymizeRampantAmountChanged(nAnonymizeRampantAmount);
            break;
        default:
            break;
        }
