...
{
    // Convert Rampant:// to Rampant:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("Rampant://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "Rampant:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Rampant.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Rampant.lnk
...
{
    return GetAutostartDir() / "Rampant.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a Rampant.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Rampant\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the Rampant app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add Rampant app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Rampant-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Rampant-qt [" + tr("command-line options") + "]                     " + "\n";
