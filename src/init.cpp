...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: Rampant.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: Rampantd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeRampantamount=<n> " + _("Keep N Rampant anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Rampant is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Rampant is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Rampant version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "Rampant server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Rampant") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Rampant to complete") << "\n";
    ...
    nAnonymizeRampantAmount = GetArg("-anonymizeRampantamount", 0);
    if(nAnonymizeRampantAmount > 999999) nAnonymizeRampantAmount = 999999;
    if(nAnonymizeRampantAmount < 2) nAnonymizeRampantAmount = 2;
    ...
    LogPrintf("Anonymize Rampant Amount %d\n", nAnonymizeRampantAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1RMT+1000 == (.1RMT+100)*10
       10RMT+10000 == (1RMT+1000)*10
    */
